...
/*   Updated: 2016/02/29 14:56:23 by                  ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_edit_zoom(double *x1, double *x2, double *y1, double *y2, 
		double *zoom, double *i_max, int keycode)
{
	if (keycode == 24) // +
	{
		*zoom += 40;
...
void		ft_mandelbrot(t_hook_info *info)
{
	...
	p = ft_make_pt(0, 0);
	ft_edit_zoom(&x1, &x2, &y1, &y2, &zoom, &i_max, info->keycode);
	...
	printf("image_x : %d\n", image_x);
	if (W_WIDTH < image_x)
		p.x = (W_WIDTH - image_x) / 2;
	while (p.x < image_x - (image_x - W_WIDTH) / 2)
	{
		if (W_HEIGHT < image_y)
			p.y = (W_HEIGHT - image_y) / 2;
		while (p.y < image_y - (image_y - W_HEIGHT) / 2)
		{
			...
			{
				if (W_WIDTH < image_x)
					ft_draw_pixel(info->current_mlx, 0x000000, ft_make_pt(p.x - (image_x - W_WIDTH) / 2,
					p.y - (image_y - W_HEIGHT) / 2));
				else
			...
			{
				if (W_WIDTH < image_x)
					ft_draw_pixel(info->current_mlx, ft_get_hexa_rgb(0, i * 255 / i_max, 0), ft_make_pt(
						p.x - (image_x - W_WIDTH) / 2, p.y - (image_y - W_HEIGHT) / 2));
				else
